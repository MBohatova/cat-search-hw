...
function fetchBreeds() {
    axios.get('/v1/breeds').then((response)=>{
        if (!response) throw Error('Oops! Something went wrong! Try reloading the page');
        return response;
    }).then((dataArr)=>{
...
function fetchCatByBreed(breedId) {
    ...
    axios.get(`v1/images/search?breed_ids=${breedId}`).then((response)=>{
        if (!response) throw Error('Oops! Something went wrong! Try reloading the page');
...
function createCatInfoBlock(data) {
    catBreedInfo.innerHTML = createCatBreedInfoBlock(data[0].height, data[0].width, data[0].url, data[0].breeds[0].alt_names, data[0].breeds[0].name, data[0].breeds[0].description, data[0].breeds[0].origin);
}
...
},{}]},["5j6Kf","a0t4e"], "a0t4e", "parcelRequirea382", {})

//# sourceMappingURL=cat-search-hw.31b563d9.js.map
